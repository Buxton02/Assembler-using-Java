...
public class Assembler
{
    ...
    private void translateOneInstruction(String line)
    {
       ...
       if(foundInstruction == "LOADN"|| foundInstruction == "LOADA"|| foundInstruction == "JMP"|| foundInstruction == "STORE"){
            //speration:
            ...
            int number = Integer.parseInt(lineSegments[1]); 
            output.println(String.format("%8s", Integer.toBinaryString(number)).replaceAll(" ", "0"));
       }
       else if (foundInstruction == "ADD"|| foundInstruction == "SUB"|| foundInstruction == "COPY") {
            //seperation
            lineSegments[1] = line.substring(length, length+1);
            lineSegments[2] = line.substring(length+2);

            //output
            output.println(instructArr[0] + reg.get(lineSegments[1]) + reg.get(lineSegments[2]));
       }
       else if (foundInstruction == "JGT"|| foundInstruction == "JLT"|| foundInstruction == "JEQ") {
            //seperation
            lineSegments[1] = line.substring(length, length+1);
            lineSegments[2] = line.substring(length+2);
            ...
            int number = Integer.parseInt(lineSegments[2]);
            output.println(String.format("%8s", Integer.toBinaryString(number)).replaceAll(" ", "0"));
       }
